<template>
  ...
  >
    <div
      class="sub-menu-content ml-6 mt-2 space-y-2 border-l border-[#ffe2d1] pl-3"
      :class="isCompactSidebar
        ? 'ml-0 space-y-1 rounded-[1.2rem] border-l-0 border border-[#ffd8c2]/80 bg-white/96 p-2 shadow-[0_18px_36px_-26px_rgba(15,23,42,0.24)] backdrop-blur'
        : ''"
    >
      <MenuItem
...
const submenuClass = computed(() => {
  if (props.isCompactSidebar) {
    return props.expanded
      ? 'absolute left-full top-0 z-30 ml-3 w-56 opacity-100'
      : 'pointer-events-none absolute left-full top-0 z-30 ml-3 w-56 opacity-0'
  }
